...
import { SavingsTableData } from '../../Components/Main/content';
...
export const SavingsTable = () => {
    ...
    const displayedData = SavingsTableData.slice(startIndex, endIndex);

    const totalPages = Math.ceil(SavingsTableData.length / itemsPerPage);
            ...
            <div>
                        ...
                        <table className="w-full table-auto text-sm text-left text-[#181336] dark:text-gray-400">
                            ...
                            <thead className="text-xs text-[#25324B] border-b">
                                <tr>
                                    ...
                                    <th scope="col" className="px-6 py-3 text-[0.825rem]">
                                        Total Saving <HiMiniChevronUpDown className='inline -mt-1' />
                                    </th>
                                    <th scope="col" className="px-6 py-3 text-[0.825rem]">
                            ...
                            {displayedData.map((item, index) => (
                                <tbody key={index}>
                                    <tr className="hover:bg-white bg-gray-50 text-[#25324B] font-medium border-b dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-600">
                ...
                <nav className="flex items-center bg-white rounded-b-lg justify-between py-4" aria-label="Table navigation">
                    <span className="text-sm font-normal px-6 text-[#515B6F] dark:text-gray-400">
                        Showing <span className="font-semibold text-gray-900 dark:text-white">{startIndex + 1}-{endIndex}</span> of <span className="font-semibold text-gray-900 dark:text-white">{SavingsTableData.length}</span>
                    </span>
