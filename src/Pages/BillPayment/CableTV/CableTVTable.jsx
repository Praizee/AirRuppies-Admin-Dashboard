...
import { CableTVTableData } from '../../../Components/Main/content';
...
export const CableTVTable = () => {
    ...
    const displayedData = CableTVTableData.slice(startIndex, endIndex);

    const totalPages = Math.ceil(CableTVTableData.length / itemsPerPage);
            ...
            <div>
                        ...
                        <table className="w-full table-auto text-sm text-left text-[#181336] dark:text-gray-400">
                            ...
                            <thead className="text-xs text-[#25324B] border-b">
                                <tr>
                                    ...
                                    <th scope="col" className="px-6 py-3 text-[0.825rem]">
                                        Plan <HiMiniChevronUpDown className='inline -mt-1' />
                                    </th>
                                    <th scope="col" className="px-6 py-3 text-[0.825rem]">
                                        Smart Number ID <HiMiniChevronUpDown className='inline -mt-1' />
                                    </th>
                                    <th scope="col" className="px-6 py-3 text-[0.825rem]">
                                ...
                                <tbody key={index}>
                                    <tr className="bg-white text-[#25324B] font-medium border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                        ...
                                        <td className="px-4 py-4 text-[0.8375rem]">
                                            <Link to="/bill-payment/cabletv-details">
                ...
                <nav className="flex items-center bg-white rounded-b-lg justify-between py-4" aria-label="Table navigation">
                    <span className="text-sm font-normal px-6 text-[#515B6F] dark:text-gray-400">
                        Showing <span className="font-semibold text-gray-900 dark:text-white">{startIndex + 1}-{endIndex}</span> of <span className="font-semibold text-gray-900 dark:text-white">{CableTVTableData.length}</span>
                    </span>
